...
class vLLMHttpServer:
    ...
    def __init__(
        ...
        self._is_teacher_model = is_teacher_model
        self._streaming_teacher = None
    ...
    async def run_server(self, args: argparse.Namespace):
        ...
        transfer_config = self.config.engine_kwargs.get("vllm", {}).get("kv_transfer_config") or {}
        if transfer_config.get("kv_connector") == "StreamOPDKVConnector":
            from verl.experimental.streamopd_kv.config import prepare_streamopd_vllm_environment

            prepare_streamopd_vllm_environment(transfer_config)
    ...
    async def stream_teacher_chunk(
        ...
        """Delegate resumable scoring to the optional Teacher service."""
        if not self._is_teacher_model:
            raise RuntimeError("stream_teacher_chunk is only available on teacher servers")
        if self._streaming_teacher is None:
            from verl.experimental.streamopd_kv.vllm_teacher import StreamingTeacherServer

            self._streaming_teacher = StreamingTeacherServer(self.engine)
        return await self._streaming_teacher.stream_teacher_chunk(token_ids, sampling_params, request_id, terminal)
