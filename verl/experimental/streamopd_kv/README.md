...
- vLLM rollout with TP=1, PP=1, and a non-quantized KV cache;
...
4. `StreamOPDKVConnector` copies completed rollout KV ranges from live vLLM pages into the host cache before EOS.
   ...
   KV lease. A bounded prefetch queue reads the next host snapshot while the current reverse unit executes, but the
   next unit is staged on GPU only after the current lease is released.
...
6. Before policy version zero, the trainer measures reusable memory once and chooses the largest safe power-of-two
   reverse width and chunk size. The plan is reused for all steps; the hot path performs no allocator query, OOM retry,
   or kernel-shape adaptation.
