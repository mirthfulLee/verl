...
class vLLMColocateWorkerExtension:
    ...
    def __new__(cls, **kwargs):
        ...
        if (getattr(vllm_config, "additional_config", {}) or {}).get("verl_exclusive_gpu_memory", False):
            from verl.experimental.streamopd_kv.vllm_patch import _configure_exclusive_gpu_memory

            _configure_exclusive_gpu_memory(vllm_config)
    ...
    def enable_streaming_prompt_logprobs(self) -> bool:
        """Install the opt-in StreamOPD teacher compatibility patch."""
        from verl.experimental.streamopd_kv.vllm_patch import enable_streaming_prompt_logprobs

        return enable_streaming_prompt_logprobs(self)
